...
class NamedArray(np.ndarray):
    ...
    def get_meta_dict(obj: T) -> dict[str, Any]:
        out = {}
        for field in obj._cls_meta_fields:
            if hasattr(obj, field.name):
                out[field.name] = getattr(obj, field.name)
            elif field.default is not None:
                out[field.name] = field.name, field.default
            elif field.default_factory is not None:
                out[field.name] = field.name, field.default_factory()
        return out
